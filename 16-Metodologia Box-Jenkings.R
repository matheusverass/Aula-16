...
ICON <- na.omit(read_excel("C:/Econometria/Icon.xls"))

Icon <-  ts(log(ICON$Fechamento), start = 2017, frequency = 365)

plot(Icon, type="l", main="Logaritmos do Preço da Icon", ylab="Log Preço", xlab="Data", col="Blue")
grid(col = "black", lty = "dotted")


#Criar FAC  e FACP

acf(log(ICON$Fechamento),lend=2, lwd=5,col="darkblue",main= "Função Autocorrelação - FAC")              #Melhorando aspecto da FAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

pacf(log(ICON$Fechamento),lend=60, lwd=5,col="darkblue",main= "Função Autocorrelação Parcial - FACP")   #Melhorando aspecto da PAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

#Teste ADF
summary(ur.df(Icon, "none", lags = 1))
...
IntOrdem1 <- diff(log(ICON$Fechamento))
IntegradaOrdem1 <- ts(IntOrdem1, start = 2017, frequency = 365)

plot(IntegradaOrdem1, type="l", main="Primeira Diferança dos Logs do Icon - LogReturn", ylab="Log Preço", xlab="Data", col="Blue")
grid(col = "black", lty = "dotted")
...
estimacoes <- list(AR1, AR2,AR3,AR4,AR5,
                   ...
                   AR16,AR17,AR18,AR19,AR20)      #Cria uma lista com os estimadores
...
Modelo <- c("AR1", "AR2","AR3","AR4","AR5",
            "AR6","AR7","AR8","AR9","AR10",
            "AR11","AR12","AR13","AR14","AR15",
            "AR16","AR17","AR18","AR19","AR20")   #cria coluna com nome dos modelos
...
plot(forecast(AR1,5), type="o", xlim=c(2017.75,2017.85), ylim=c(-0.03,0.06))
grid(col = "black", lty = "dotted")
